            ...
            switch (current_state) {
                ...
                case UNSIGNED_INTEGER_STATE: {
                    ...
                    if(ch == EOF){
                        uint64_t unInt;
                        if( ss >> unInt){
                            return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, unInt, pos, currentPos()), std::optional<CompilationError>());
                        }
                        else{
                            result =  std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrStreamError));
                            return result;
                        }
                    }
                ...
                case IDENTIFIER_STATE: {
                    ...
                    if(ch == EOF){
                        ...
                        else return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrStreamError));
                    }
